...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:lambda:us-east-1:193871648423:function:supply-chain-*"
      ]
    }
  ]
}
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "aws:SourceAccount": "193871648423"
        }
      }
    }
  ]
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:redshift-serverless:us-east-1:193871648423:workgroup/*",
        "arn:aws:redshift-serverless:us-east-1:193871648423:namespace/*"
      ]
    },
    {
      "Effect": "Allow",
      ...
      "Resource": "arn:aws:logs:us-east-1:193871648423:log-group:/aws/lambda/*"
    }
  ]
}
```
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:s3:::supply-chain-data-193871648423",
        "arn:aws:s3:::supply-chain-data-193871648423/*"
      ]
    },
    {
    ...
    },
    {
      ...
      "Resource": "arn:aws:logs:us-east-1:193871648423:log-group:/aws-glue/*"
    }
  ]
}
```
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:bedrock:us-east-1:193871648423:agent/*",
        "arn:aws:bedrock:us-east-1::foundation-model/anthropic.claude-3-5-sonnet-20241022-v2:0"
      ]
    },
    {
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:lambda:us-east-1:193871648423:function:*"
      ]
    }
  ]
}
...
**Configure Bucket**:
1. **Bucket name**: Enter "supply-chain-data-193871648423"
...
| Key | Value |
|-----|-------|
| --S3_BUCKET | supply-chain-data-193871648423 |
...
We need to create 11 Lambda functions (agent tools). Lambda functions are individual Python files, not directories. We'll use an automated Python script to package them with dependencies.

**Important Note**: The Lambda functions are individual `.py` files located in the `tools/` folders, not separate directories. We'll package them using the automated script.

### Step 6.1: Package Lambda Functions with Automated Script
...
📦 Packaging calculate_forecast...
   ✓ Copied source file
   ⏳ Installing dependencies: boto3, pandas, numpy, statsmodels
   ✓ Dependencies installed
   ✓ Created: lambda_packages\calculate_forecast.zip (18.45 MB)
...
**Important**: You'll have two files named `lambda_function.zip`. Rename them:
- Rename `lambda/inventory_agent/lambda_function.zip` → `inventory_agent.zip`
- Rename `lambda/metrics_calculator/lambda_function.zip` → `metrics_calculator.zip`
...
3. Create folder structure:
   ...
   - Inside `lambda/`, create subfolders:
     - `forecasting`
     - `procurement`
     - `inventory`
     - `metrics`
...
1. Navigate to `supply-chain-data-193871648423/lambda/forecasting/`
2. Click "Upload"
3. Click "Add files"
4. Select these files from `lambda_packages/`:
   - get_historical_sales.zip
   - calculate_forecast.zip
   - store_forecast.zip
   - calculate_accuracy.zip
...
1. Navigate to `supply-chain-data-193871648423/lambda/procurement/`
2. Click "Upload"
...
1. Navigate to `supply-chain-data-193871648423/lambda/inventory/`
2. Click "Upload"
3. Select: `inventory_agent.zip`
4. Click "Upload"

**Upload to `lambda/metrics/`:**
1. Navigate to `supply-chain-data-193871648423/lambda/metrics/`
2. Click "Upload"
3. Select: `metrics_calculator.zip`
4. Click "Upload"

**Verify Uploads**:
- Check each folder has the correct .zip files
- Total: 11 .zip files across 4 folders
...
### Step 6.4: Lambda Function Details

**Create these 11 Lambda functions using the template above**:
...
- Keep the Lambda console open in one tab
- Open the table above in another window for reference
- Copy-paste function names and S3 URIs directly
- All functions use the same configuration (role, env vars, timeout)
- Create them in order from 1-11

### Step 6.5: Test a Lambda Function

**Test the first function to verify everything works**:
...
**Expected Result**: Function executes successfully (may return empty data if Glue ETL hasn't loaded data yet)

### Step 6.6: Verify All Functions

**Final Verification**:

1. In Lambda console, you should see **11 functions**
2. All should have status **"Active"**
3. All should use **SupplyChainToolRole**
4. All should have **3 environment variables**
5. All should have **512 MB memory** and **15 min timeout**

**Verification Checklist**:
- [ ] Script ran successfully (11 functions packaged)
- [ ] All .zip files exist in `lambda_packages/` directory
- [ ] All .zip files uploaded to S3 in correct folders
...
✅ **All 11 Lambda Functions Created!**

**Troubleshooting**:

**Issue: "No module named 'pandas'"**
- Cause: Dependencies not included in zip
- Solution: Delete .zip file, run `python package_lambdas.py` again

**Issue: Script fails with "pip not found"**
- Cause: pip not in PATH
- Solution: Run `python -m pip install --upgrade pip`

**Issue: Function returns error when tested**
- Cause: Redshift data not loaded yet
- Solution: Ensure Glue ETL job completed successfully in Phase 5
